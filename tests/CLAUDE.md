...
- `test_ui_mode.py`: #180 Simple/Advanced view. AppSettings `ui_mode` contract (default 'simple', round-trip, unknown coerces), `SimpleModeWidget` signal/`set_busy` behavior, and `MainWindow._apply_ui_mode` page-swap driven on a lightweight stub `self` (no full window — no pytest-qt). Imports `AppSettings` via `src.utils.settings` so the monkeypatch hits the same class object `main_window` uses. Also covers the #397 fix: `SimpleModeWidget.set_apply_dirty` mirrors the Advanced-mode Apply button's red/green convention (previously hardcoded green regardless of state) — starts dirty by construction, color always tracks `set_apply_dirty`, and a run in progress (`set_busy(True)`) always wins for enabled state even if dirty flips mid-run, without disturbing what color it reports.
...
- `test_apply_already_applied.py`: #387. `_matches_applied_output(stock_dict, merged_dict, applied_dict)` — the Qt-free comparison behind the Apply-button dirty check — only ever compares `stock_dict`'s own keys, since `merge_ini_files` (the real writer) never writes a key the structure-preservation source file (stock `base.ini`) lacks regardless of what `merged_dict` carries for it. Covers untouched stock passthrough, an override already written vs. not yet written, a merged-only key outside `stock_dict` being correctly ignored either way, and a key missing from the applied file entirely. Also covers the #397 follow-up fix: `MainWindow._refresh_apply_dirty_after_reload` (driven on a stub `self`, same pattern as `test_install_scan_finished_stale_worker.py`) has to run this check on *every* reload, not just the first at startup — Simple mode's one-button flow reloads immediately after a successful `apply_to_game()` to refresh the hidden Advanced view, and that reload's own unconditional dirty-marking was undoing what `apply_to_game()` had just correctly cleared, both the button color and the exit-time "unapplied changes" warning.
