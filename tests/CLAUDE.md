...
- `test_mission_route_title.py`: #166 hauling/delivery route-in-title. The pure `_derive_route_fragment` / `_route_token_role` / `_title_route_token` helpers in `generate_enhancements_ini.py` driven with synthetic bodies (A→B → `from > to`, MultiToSingle → `to`, SingleToMulti → `from`, Pickup/Dropoff copied verbatim, ambiguous multi/multi omitted), plus the AppSettings contract for the new `route` mission-detail toggle. Deliberately does NOT use the stale `kraken_4.7.ini` fixture.
