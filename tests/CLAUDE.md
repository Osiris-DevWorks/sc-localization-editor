...
- `test_apply_already_applied.py`: #387. `_matches_applied_output(stock_dict, merged_dict, applied_dict)` — the Qt-free comparison behind the startup Apply-button dirty check — only ever compares `stock_dict`'s own keys, since `merge_ini_files` (the real writer) never writes a key the structure-preservation source file (stock `base.ini`) lacks regardless of what `merged_dict` carries for it. Covers untouched stock passthrough, an override already written vs. not yet written, a merged-only key outside `stock_dict` being correctly ignored either way, and a key missing from the applied file entirely.
