...
- `test_blueprint_auto_scan.py`: #386 opt-in startup auto-scan for "Scan Logs for Owned Blueprints". `AppSettings.get_/set_auto_scan_blueprints_enabled()` contract (default False); `MainWindow._maybe_auto_scan_blueprints` driven on a stub `self` (same pattern as `test_favorite_toggle_stranded`) — gated off when the setting is off (even with a valid install path, to isolate the setting itself as the gate) or the active channel has no valid install path yet (silently, no warning dialog unlike the manual scan), skipped when a scan is already running, and sets `_bp_scan_silent = True` + never forces a full rescan when it does start; `MainWindow._finish_blueprint_scan_queue`'s silent branch swaps the manual scan's modal summary dialogs for a status bar message, still writes the owned set and re-weaves `[Owned]` tags, and consumes the one-shot flag whether or not anything new was found.
