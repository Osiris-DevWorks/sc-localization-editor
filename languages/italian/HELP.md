...
- Clicca sulla colonna **★** su qualsiasi riga di nave per contrassegnarla come preferita.
...
## 13. Dopo gli aggiornamenti del gioco
...
- **Campi Dettagli Missione** — mostra o nascondi individualmente ogni riga del blocco MISSION DETAILS (tipo di missione, difficoltà, spawn, reputazione, blueprint, e il tag del titolo `[BP]`), così le tue descrizioni di missione riportano solo i dati che ti interessano.
...
- **Scansiona anche LIVE/HOTFIX (quello non attivo)** controlla anche quello dei due che non è il tuo canale attuale, dato che condividono la stessa progressione dell'account — un blueprint ottenuto su LIVE compare nei log di HOTFIX e viceversa. Disattivato per impostazione predefinita (l'intera cronologia dei log di un secondo canale è lavoro extra da leggere). PTU, EPTU e TECH-PREVIEW sono build di test separate con una propria progressione e non vengono mai scansionate, indipendentemente da questa opzione.
