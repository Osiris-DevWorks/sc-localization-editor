...
| Change stock-title shortening (Mission Titles) | `src/utils/tag_builder.py`, `scripts/generate_enhancements_ini.py` | `TITLE_ABBREVIATIONS` phrase map + `abbreviate_title()`; gated by `TagConfig.abbreviate_title` (off by default), applied at both title-emission sites (#200 follow-up) |
