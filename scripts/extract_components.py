...
Compares the current base.ini (downloaded by Open Strings) against the
...
The output file can then be configured as the 'Components' source in Open Strings
so the hierarchy is: stock global → components (mods) → contracts → user.
...
def main() -> None:
    ...
    if not base_path.exists():
        ...
        print("  Run Open Strings first to download the base file, or pass --base <path>.")
        sys.exit(1)
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("; Open Strings - extracted components\n")
    ...
    print("\nNext steps:")
    print("  1. In Open Strings > Config tab, set the Components source to:")
    print(f"     {output_path}")
