...
- **Auto-scan logs on startup** runs the same scan automatically every time Smart Citizen starts, so your Owned list stays current without a manual click. Off by default. It reports quietly through the status bar instead of a popup, so a launch that finds nothing new stays silent.
